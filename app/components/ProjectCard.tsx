    ...
    >
      ...
      >
        {index !== undefined && (
          <span
            aria-hidden
            className="hidden w-5 flex-shrink-0 font-mono text-[10px] tabular-nums text-zinc-600 sm:block"
          >
            {String(index + 1).padStart(2, "0")}
          </span>
        )}

        <span className="grid h-8 w-8 flex-shrink-0 place-items-center rounded-lg border border-accent/20 bg-accent/10 text-accent-300">
          <FolderGit2 className="h-3.5 w-3.5" />
        </span>

        <span className="min-w-0 flex-1 text-left">
          <span className="flex items-center gap-2">
            <span className="line-clamp-1 text-sm font-semibold text-zinc-100 group-hover/trigger:text-accent-200 sm:text-[15px]">
              {project.title}
            </span>
            {hasLink && (
              <span className="inline-flex flex-shrink-0 items-center gap-0.5 rounded border border-accent/20 bg-accent/10 px-1.5 py-px text-[9px] font-bold uppercase text-accent-300">
          ...
          <span className="mt-0.5 flex items-center gap-2 text-[11px] text-zinc-500">
            ...
            {category && (
              <>
                ...
                <span className="flex-shrink-0 font-mono uppercase tracking-wider text-zinc-600">
                  {category}
                </span>
              </>
            )}
        ...
        <span
          className={[
            "grid h-7 w-7 flex-shrink-0 place-items-center rounded-md border border-border/80 bg-surface/60 text-zinc-500 transition-all duration-200",
            isOpen
              ? "rotate-180 border-accent/40 bg-accent/10 text-accent-300"
              : "group-hover/trigger:text-accent-300",
          ].join(" ")}
          aria-hidden
        >
            ...
            <div className="faq-panel space-y-3">
              <div className="grid gap-3 md:grid-cols-2">
                <div className="card-inner p-3">
                  <p className="mb-1.5 font-mono text-[10px] uppercase tracking-[0.16em] text-zinc-500">
                    Overview
                  </p>
                ...
                {project.keyTakeaway && (
                  <div className="rounded-lg border border-accent/20 bg-accent/[0.06] p-3">
                    <p className="mb-1.5 flex items-center gap-1.5 font-mono text-[10px] font-semibold uppercase tracking-[0.16em] text-accent-300">
              ...
              {project.categories && project.categories.length > 1 && (
                <div className="flex flex-wrap gap-1.5">
                  {project.categories.map((cat) => (
                    <span
                      key={cat}
                      className="rounded border border-border/60 bg-surface/50 px-2 py-0.5 font-mono text-[10px] uppercase text-zinc-500"
                    >
                      {cat}
                    </span>
                  ))}
